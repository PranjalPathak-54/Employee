...
const Employees = () => {
  ...
  const fetchEmployees=useCallback(async()=>{
    try{
      const url=selectdept ? `/employee?department=${selectdept}`:`/employee`
  ...
  const filtered=employee.filter((emp)=>`${emp.firstName} ${emp.lastName} ${emp.position}`.toLowerCase().includes(search.toLowerCase()))
  return (
    <div className='animate-fade-in'>
      <div className='flex flex-col sm:flex-row justify-between item-start sm:items-center gap-4 mb-8'>
        <div>
      ...
      </div>
      {
        loading ? (
          <div className='flex justify-center p-12'>
            <div className='animate-spin h-8 w-8 border-2 border-indigo-600 border-t-transparent rounded-ful'/>
          </div>
        ):(
          <div className='grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 sm:p-5'>
            {
              filtered.length===0 ? (
      ...
      {showCreateModel && (
        <div className='fixed bg-black/40 backdrop-blur-sm inset-0 z-50 flex items-start justify-center p-4 overflow-y-auto' onClick={()=>setshowCreateModel(false)}>
          ...
          <div className='relative bg-white rounded-2xl shadow-2xl w-full max-w-3xl my-8 animate-fade-in' onClick={(e)=>e.stopPropagation()}>
            ...
            <div className='p-6'>
              <EmployeeForm onSuccess={()=>{setshowCreateModel(false);fetchEmployees();}} onCancel={()=>setshowCreateModel(false)}/>
            </div>
          </div>
        </div>
      )}
          ...
          <div className='relative bg-white rounded-2xl shadow-2xl w-full max-w-3xl my-8 animate-fade-in' onClick={(e)=>e.stopPropagation()}>
            <div className='flex items-center justify-between p-6 pb-0'>
              <div>
                ...
                <p className='text-sm text-slate-500 mt-0.5'>Edit employee profile</p>
              </div>
