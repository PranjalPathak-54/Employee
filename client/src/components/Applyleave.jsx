...
const Applyleave = ({open,onClose,onSuccess}) => {
  ...
  const handleSubmit=async(e)=>{
    ...
    try{
      const formData=new FormData(e.currentTarget)
      await api.post('/leave',Object.fromEntries(formData))
    ...
    <div className='fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm' onClick={onClose}>
        <div className='relative bg-white rounded-2xl shadow-2xl w-full max-w-lg animate-fade-in' onClick={(e)=>e.stopPropagation()}>
            <div className='flex items-center justify-between p-6 pb-0'>
                ...
                <button type="button" onClick={onClose} aria-label="Close" className='p-2 rounded-lg hover:bg-slate-100 transition-colors text-slate-400 hover:text-slate-600'>
                    <X className='w-5 h-5'/>
                </button>
            </div>
            <form onSubmit={handleSubmit} className='p-6 space-y-5'>
                ...
                <div>
                    ...
                    <textarea name='reason' required rows={3} className='resize-none' placeholder="Reason for your leave"/>
                </div>
